...
Ground truth for the `elite_wars` OCR parser tests.

Elite Wars uses the same layout as Polar Invasion (`PolarInvasionV1Parser`).
...
Example: `20260421T1500_001.jpg` / `20260421T1500_001.json`
...
by the parser). The code `"elite_wars"` is assigned by the dispatcher upstream.
The `total_battlers` and `total_points` fields can be `null` if missing from
the header (e.g. Ironblood screenshots without these values).
