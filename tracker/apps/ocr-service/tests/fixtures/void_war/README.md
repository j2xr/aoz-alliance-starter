...
Ground truth for the `void_war` OCR parser tests.

Void War uses the same layout as Polar Invasion (`PolarInvasionV1Parser`).
...
by the parser). The code `"void_war"` is assigned by the dispatcher upstream.
