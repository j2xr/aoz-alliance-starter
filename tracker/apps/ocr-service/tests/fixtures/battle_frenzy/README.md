...
Ground truth for the `battle_frenzy` OCR parser tests.

Battle Frenzy uses the same layout as Polar Invasion (`PolarInvasionV1Parser`).
...
by the parser). The code `"battle_frenzy"` is assigned by the dispatcher upstream.
