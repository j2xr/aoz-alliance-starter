"""Integration tests: parser output vs. ground-truth JSON fixtures.

Each fixture JSON is paired with its companion JPG (same stem).
The test calls preprocess_image() + PolarInvasionV1Parser.parse() and
compares the result field-by-field.
...
def test_parser_matches_fixture(fixture_path: Path) -> None:
    with fixture_path.open(encoding="utf-8") as fh:
        expected = json.load(fh)

    # Image has same stem as the JSON (e.g. 20260407T1500_001.jpg)
